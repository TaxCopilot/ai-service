...
def generate_notice_reply(
    ...
    Raises RuntimeError if the model call fails or the response can't be parsed.
    """
    llm = _build_llm()

    law_context = retrieved_law.strip() or (
        "No specific law excerpts retrieved. Apply general knowledge of the Income Tax Act 1961."
    )

    messages = [
        SystemMessage(content=_SYSTEM_PROMPT),
        HumanMessage(content=_USER_PROMPT.format(notice_text=extracted_text, law_excerpts=law_context)),
    ]
